...
const addressHandler = (address) => {
  ...
  if (address.length === 0) {
    ...
    $('#address-error').classList.remove('d-none')
  ...
  addressHtmlList.append(
    "<p class='text-center'><strong>Информация об адресе</strong></p>"
  )
...
const metroHandler = (metroList) => {
  ...
  metroHtmlList.append(
    "<p class='text-center'><strong>Список станций метро</strong></p>"
  )
...
const mapHandler = (address, metroList) => {
  ...
  const userPositionMarker = L.marker([
    address.longitude[1],
    address.latitude[1],
  ]).addTo(map)
